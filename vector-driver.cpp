...
int main(int argc, char **argv) {
  ...
  if (test_num == 0) { for (int i = 0; i < num_of_tests; i++) { Tests[i](); } } 
  else if (test_num > 0 && test_num <= num_of_tests) { Tests[test_num - 1](); }
}
...
void TestSubscripts() {
  ...
  for (hlp2::vector::value_type i{}; i < max_cnt; i++) { a.push_back(2 * i); }
  Print(a);

  std::cout << "multiply each value by 3:\n";
  for (hlp2::vector::value_type i{}; i < max_cnt; i++) { a[i] *= 3; }
  Print(a);

  std::cout << "use subscript with non-default constructed vector:\n";
  hlp2::vector b(10);
  for (hlp2::vector::size_type i{}; i < b.size(); ++i) {  b[i] = 3*i; }
  Print(b);

  std::cout << "push back 10 odd ints:\n";
  hlp2::vector c;
  for (int i{}; i < 10; ++i) { c.push_back(-i*2+3); }
  Print(c);
  ...
  for (hlp2::vector::size_type i{}; i < c.size(); ++i) { c[i] = d[i]-1; }
  Print(c);
}
...
void TestCopy() {
  ...
  std::cout << "push_back 10 even integers:\n";
  for (int i{}; i < 10; i++) { a.push_back(2 * i); }
...
void TestALot1() {
  ...
  for (int i{}; i < 10; i++) { a.push_back(2 * i + 1); }
  Print(a);

  std::cout << "empty/fill with 10 ints 3 times:\n";
  for (int i{}; i < 3; i++) {
    a.resize(0);
    for (int j{}; j < 10; j++) { a.push_back(j); }
  }
  ...
  for (int i{}; i < 8; i++) { a.push_back(i); }
  Print(a);
}

void Print(hlp2::vector const& array, std::string const& newline) {
  for (int x : array) { std::cout << x << " "; }
  std::cout << "(size=" << array.size() << ", capacity=" << array.capacity() << ", allocs=" << array.allocations() << ")";
  std::cout << newline;
}
